...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def initialize_parameters(
        ...
        self.before_round_global_params = None
    ...
    def aggregate_fit(
        ...
        for client_id, valloader in enumerate(self.valloaders):
            ...
            for client_proxy, fit_res in results:
                ...
                if idx != int(client_id):
                    model = Net().to(DEVICE)
                    if idx == NUM_CLIENTS-2 or idx == NUM_CLIENTS-1:
                        #TODO:Deltaフリーライダーの実装
                        globa_params = parameters_to_ndarrays(self.current_round_global_params)
                        if self.before_round_global_params is None:
                            print(f"(Random) Client {idx}  Random times: {random_times} (Random param)")
                            delta_param = [param + (param * random_times) for param in globa_params]
                        else:
                            before_params = parameters_to_ndarrays(self.before_round_global_params)
                            delta_param = [param + (param - before) for param, before in zip(globa_params, before_params)]
                            for param, before,delta in zip(globa_params, before_params,delta_param):
                                if np.array_equal(param + (param - before), delta):
                                    print(f"(Delta) Client {idx}  Delta prama is success (Delta param)")
                                    break
                                else:
                                    print(f"(Delta) Client {idx}  Delta is fail (Delta param)")
                                    break
                            d_params = globa_params - before_params
                            d_params += globa_params
                            if np.array_equal(d_params, delta_param):
                                print(f"(Delta) Client {idx}  Delta prama is success (Delta param)")
                                break
                        set_parameters(model, delta_param)
                    else:
                        set_parameters(model, parameters_to_ndarrays(fit_res.parameters))
                    loss, accuracy = evaluate_model_on_own_data(model, valloader)
                    tmp_loss_for_client[int(idx)] = loss
                    tmp_accuracy_for_client[int(idx)] = accuracy
                    if loss < GLoss:
                        # クライアントごとのLossとAccuracyを提出する
                        clinet_accuracy[int(idx)].append(accuracy)
                        clinet_loss[int(idx)].append(loss)
        ...
        #     print(f"Next Round Epochs: {EPOCHS}")
            ...
            #一つ前のグローバルモデルを更新
            self.before_round_global_params = self.current_round_global_params
...
def plot_client_token_history(SAVE_DIR):
    # 利用可能なスタイルを確認
    print(plt.style.available)

    # ファイルパスの確認
    file_path = f'{SAVE_DIR}/clinet_token_history.csv'
...
if __name__ == "__main__":
    ...
    now = datetime.datetime.now()
    current_time = now.strftime("%Y-%m-%d-%H-%M")
    SAVE_DIR = f"DeltaFreeRider/{current_time}"
    ...
    plot_client_token_history(SAVE_DIR)
