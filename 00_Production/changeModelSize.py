...
def load_datasets_stratified(num_clients:int, client_data_sizes:List):
    """
...
class Net(nn.Module):
    def __init__(self) -> None:
        ...
        self.fc1 = nn.Linear(512 * 4 * 4, 1024)  # CIFAR-10の画像サイズ(32x32)を考慮
            ...
            for client_proxy, fit_res in results:
                if int(client_proxy.cid) != int(client_id):
                    ...
                    if loss < GLoss:
                        ...
                        clinet_accuracy[int(client_proxy.cid)].append(accuracy)
                        clinet_loss[int(client_proxy.cid)].append(loss)
                        tmp_accuracy.append(accuracy)
                        tmp_loss.append(loss)
                        own_test_results.append((client_proxy.cid, loss, accuracy))
                        print(f"Client {client_id} -> Client {client_proxy.cid}  Accuracy: {accuracy:.4f}, Loss: {loss:.4f}  (Submitted)")
                    else:
...
def plot_client_token_history(current_time:str):
    ...
    # ファイルパスの確認
    file_path = f'ChangeModelSize/{current_time}/clinet_token_history.csv'
    ...
    # グラフの保存
    plt.savefig(f'ChangeModelSize/{current_time}/0_client_token_amount_history.png', bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    now = datetime.datetime.now()
    current_time = now.strftime("%Y-%m-%d-%H-%M")
    SAVE_DIR = f"ChangeModelSize/{current_time}"
